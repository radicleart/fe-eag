...
export default {
  ...
  props: ['loopRun', 'mode', 'displayClass'],
  data () {
    return {
      hideForCPS: true,
  ...
  mounted () {
    this.query.claims = (this.loopRun.currentRunKey === 'crash_punks') ? 'claimed' : null
    const data = {
  ...
  methods: {
    ...
    unclaimed () {
      if (this.query.claims === 'unclaimed') {
        this.query.claims = 'claimed'
      } else {
        this.query.claims = 'unclaimed'
      }
      this.toggleSearching()
    },
    showUnclaimed () {
