...
<b-container>
  <b-row class="p-5 row mt-2">
    <b-col cols="12">
      <h1><span class="mr-5 text-black">Buy Now</span> {{contractAsset.listingInUstx.price  + ' ' + contractAsset.listingInUstx.symbol}}</h1>
    </b-col>
...
export default {
  ...
  methods: {
    rateMessage: function () {
      return 'Buy now for' + formatUtils.fmtAmount(this.contractAsset.listingInUstx.price, 'stx') + ' ' + this.contractAsset.listingInUstx.symbol
    }
  },
  computed: {
    profile () {
